...
class InputExample(object):
    ...
    def __init__(self, guid, text_a, text_b=None, label=None, time_v=None):
...
class MyTaskProcessor(DataProcessor):
    ...
    def get_dev_examples(self, data_dir):
        return self._create_examples(
            self._read_tsv(os.path.join(data_dir, 'test.txt')), 'val')

    def get_test_examples(self, data_dir):
        return self._create_examples(
            self._read_tsv(os.path.join(data_dir, 'pred.txt')), 'test')
    ...
    def _create_examples(self, lines, set_type):
        ...
        for (i, line) in enumerate(lines):
            if i == 0:
                continue
            guid = '%s-%s' % (set_type, i)
            text_a = tokenization.convert_to_unicode(line[3] + line[1])
...
class MyTaskProcessor(DataProcessor):
    ...
    def get_train_examples(self, data_dir):
        lines = self._read_csv(data_dir, "train1.csv")
        examples = []
        for (i, line) in enumerate(lines):
            if i == 0:
                continue
    ...
    def get_dev_examples(self, data_dir):
        lines = self._read_csv(data_dir, "test1.csv")
        examples = []
        for (i, line) in enumerate(lines):
            if i == 0:
                continue
    ...
    def get_test_examples(self, data_dir):
        ...
        for (i, line) in enumerate(lines):
            ...
            # text_a = tokenization.convert_to_unicode(line[0]) + ' ' + ''.join(list(map(str, bc.encode([str(line[2])]))))
...
def create_model(bert_config, is_training, input_ids, input_mask, segment_ids,
    ...
    '''
  vec = []
  for i in range(temp, temp + 16):
      veclist = []
      for j in time_v[i].split(' '):
          veclist.append(float(j))
      vec.append(veclist)

  temp += 16

  tf.concat([output_layer, vec], 1)  # 拼接
  '''
    ...
    with tf.variable_scope("loss"):
        ...
        # output_layer = tf.concat([output_layer, vec], 0)  # 拼接

        # output_weights = tf.reshape(output_weights, [64, 24])
        ...
        if mode == tf.estimator.ModeKeys.TRAIN:
            ...
            def metric_fn(per_example_loss, label_ids, logits, is_real_example):
                ...
                f1_score, f1_update_op = tf.metrics.mean(
                    (2 * precision_update_op * recall_update_op) / (precision_update_op + recall_update_op),
                    name='f1_score')
                return {
